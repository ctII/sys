...
func semctl(id int, num int, cmd uint, arg int) (result int, err error) {
	r0, _, e1 := Syscall6(SYS_SEMCTL, uintptr(id), uintptr(num), uintptr(cmd), uintptr(arg), 0, 0)
	result = int(r0)
	if e1 != 0 {
		err = errnoErr(e1)
	}
...
func semget(key int, nsems int, flag int) (result int, err error) {
	r0, _, e1 := Syscall(SYS_SEMGET, uintptr(key), uintptr(nsems), uintptr(flag))
	result = int(r0)
	if e1 != 0 {
		err = errnoErr(e1)
	}
...
func semtimedop(id int, tsops []SysvSemBuf, nsops uint, timeout *Timespec) (result int, err error) {
	var _p0 unsafe.Pointer
	if len(tsops) > 0 {
		_p0 = unsafe.Pointer(&tsops[0])
	} else {
		_p0 = unsafe.Pointer(&_zero)
	}
	r0, _, e1 := Syscall6(SYS_SEMTIMEDOP, uintptr(id), uintptr(_p0), uintptr(len(tsops)), uintptr(nsops), uintptr(unsafe.Pointer(timeout)), 0)
	result = int(r0)
	if e1 != 0 {
		err = errnoErr(e1)
	}
...
func semop(id int, tsops []SysvSemBuf, nsops uint) (result int, err error) {
	var _p0 unsafe.Pointer
	if len(tsops) > 0 {
		_p0 = unsafe.Pointer(&tsops[0])
	} else {
	...
	r0, _, e1 := Syscall6(SYS_SEMOP, uintptr(id), uintptr(_p0), uintptr(len(tsops)), uintptr(nsops), 0, 0)
	result = int(r0)
	if e1 != 0 {
		err = errnoErr(e1)
	}
